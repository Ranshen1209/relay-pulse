...
describe('hasAnyAnnotation', () => {
  ...
  it('returns true when item has annotations', () => {
    const item = mockItem({
      annotations: [makeAnnotation({ id: 'custom_note', family: 'neutral' })],
    });
    expect(hasAnyAnnotation(item)).toBe(true);
  });

  it('returns false when item only has hidden annotations', () => {
    const item = mockItem({
      annotations: [
        makeAnnotation({ id: 'public_service', family: 'neutral' }),
        makeAnnotation({ id: 'sponsor_beacon', family: 'positive' }),
      ],
    });
  ...
  it('defaults enableAnnotations to true', () => {
    const item = mockItem({
      annotations: [makeAnnotation({ id: 'custom_note', family: 'positive' })],
    });
    expect(hasAnyAnnotation(item)).toBe(true);
  });
});
